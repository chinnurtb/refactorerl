...
update(Node, Data) when element(1, Node) =:= ?NODETAG ->
    case gen_server:call(?GRAPH_SERVER, {update, Node, Data}) of
        ok    -> ok;
...
delete(Node) when element(1, Node) =:= ?NODETAG ->
    gen_server:call(?GRAPH_SERVER, {delete, Node}).

%% @spec data(node()) -> data()
%% @doc Returns the data associated with a node.
data(Node) when element(1, Node) =:= ?NODETAG ->
    case gen_server:call(?GRAPH_SERVER, {data, Node}) of
        {data, Data}   -> Data;
...
mklink(From, Tag, To) when element(1, From) =:= ?NODETAG,
                           element(1, To) =:= ?NODETAG,
                           is_atom(Tag); is_tuple(Tag) ->
    case gen_server:call(?GRAPH_SERVER, {mklink, From, Tag, To}) of
...
rmlink(From, Tag, To) when element(1, From) =:= ?NODETAG,
                           element(1, To) =:= ?NODETAG,
                           is_atom(Tag) ->
    case gen_server:call(?GRAPH_SERVER, {rmlink, From, Tag, To}) of
        ok -> ok;
...
links(Node) when element(1, Node) =:= ?NODETAG ->
    gen_server:call(?GRAPH_SERVER, {links, Node}).

%% @spec path(node(), path()) -> [node()]
%% @doc Evaluates a path expression starting from Node, and returns the
%% resulting nodes.
path(Node, Path) when element(1, Node) =:= ?NODETAG,
                      length(Path) > 0 ->
    case gen_server:call(?GRAPH_SERVER, {path, Node, Path}) of
...
index(From, Tag, To) when element(1, From) =:= ?NODETAG,
                          element(1, To) =:= ?NODETAG,
                          is_atom(Tag) ->
    case gen_server:call(?GRAPH_SERVER, {index, From, Tag, To}) of
        bad_link -> erlang:error(bad_link, [From, Tag, To]);
        Reply    -> Reply
    end.
...
restore(Count) ->
  gen_server:call(?GRAPH_SERVER, {restore, Count}).

%% @spec undo() -> restored | invalid_checkpoint_number
%% @doc Restores the previous state of the database from the last
%% checkpoint. The actual checkpoint numbers are
%% stored in the database.
undo() ->
  gen_server:call(?GRAPH_SERVER, {undo}).
...
%% stored in the database.
redo() ->
  gen_server:call(?GRAPH_SERVER, {redo}).

%% @spec clean() -> cleaned_all_backups
%% @doc Cleans all backup files from the mnesia storage
clean()->
  gen_server:call(?GRAPH_SERVER, {clean}).

%%% ==================================================================
%%% Implementation
%%%
%%% For each node class, there are three mnesia tables:
...
%%%  - one with '$back' postfix that stores bacward link information
%%%
%%% Every node has an integer ID, which is unique in its class. A node
%%% is represented by a [Class|Id] term.
%%%
...
%%% where Class is the name of the table, From and To are ID-s, Tag is
%%% the link tag and Index is the index of the link among the links from the
%%% same node and with the same tag. The (non-unique) key is the first
%%% element.
%%% The backward link table contains tuples of the form
%%%   {Class, {To, Tag}, From}.
...
handle_call({undo}, _F, S) ->
    ...
    if IdAkt > 0 -> restorech(Id),
                    Reply = undo_is_ok,
                    mnesia:dirty_write({counter,1,IdAkt -1});
             true -> Reply = invalid_checkpoint_number
    end,
    {reply, Reply, S};
...
delete_forward_links(Class, Id) ->
    [ mnesia:dirty_delete_object(linktab(Class), Rec) ||
        Rec <- mnesia:dirty_match_object(linktab(Class),
                                         {'_',{Id,'_'},'_','_'})].
...
delete_backward_links(Class, Id) ->
    ...
    [ mnesia:dirty_delete_object(Rec) ||
        {FCl, Tags} <- Links,
        Rec = {_,{_,FTag}, _,_} <-
            mnesia:dirty_match_object({linktab(FCl), '_', '_', Id}),
        lists:member(FTag, Tags) ].

tabinfo({Tag, Tables}, WS) ->
    {Tag,
     lists:sum([mnesia:table_info(T, size) || T <- Tables]),
     lists:sum([WS*mnesia:table_info(T, memory) || T <- Tables])}.
...
insert_link(Links, From, To, Tag, Ind) ->
    ...
    mnesia:dirty_delete(Links, {From,Tag}),
...
count()->
    [{_, _, Id}] = mnesia:dirty_read({counter, 0}),
    %[{_, _, IdAkt}] = mnesia:dirty_read({counter, 1}),
    %mnesia:dirty_write({counter, 0, Id + 1}),
    %mnesia:dirty_write({counter, 1, Id + 1}),
    Id+1.

checkpoint()->
    Name=atom_to_list(backup)++"."++integer_to_list(count()),
    Tab = mnesia:system_info(tables),
    Args = [{name,Name},{max, Tab},
    {allow_remote,true},{ram_overrides_dump,true}],
    _D = (catch mnesia:activate_checkpoint(Args)),
    Dir=mnesia:system_info(directory),
    File=filename:absname_join(Dir,Name),
    _C = (catch mnesia:backup_checkpoint(Name,File)),
    mnesia:deactivate_checkpoint(Name),
    mnesia:dirty_write({counter,0,count()}),
    [{_,_,IdAkt}] = mnesia:dirty_read({counter,1}),
    mnesia:dirty_write({counter,1,IdAkt + 1}),
    {ok, Name}.
...
compile_path([Elem | Rest], Class, Query) ->
    ...
    Cond = 
        case Filter of
            {} ->
                fun(_Ind, _Id) -> true end;
            Index when is_integer(Index),
                       Dir =:= fwd ->
                fun(Ind, _Id) -> Ind =:= Index end;
            last when Dir =:= fwd ->
                fun
                    (Ind, Id) -> 
            ...
            {Ind1, last} when is_integer(Ind1),
                              Dir =:= fwd ->
                fun(Ind, _Id) -> Ind >= Ind1 end;
            {Ind1, Ind2} when is_integer(Ind1),
                              is_integer(Ind2),
                              Dir =:= fwd ->
                fun(Ind, _Id) -> Ind >= Ind1 andalso Ind < Ind2 end;
            _ ->
...
compile_filter(AttrInfo, Attrs, {Filt1, Op, Filt2})
  when Op =:= 'and' orelse Op =:= 'or' ->
    ...
    case Op of
        'and' -> fun(Ind, Id) -> F1(Ind, Id) andalso F2(Ind, Id) end;
        'or' ->  fun(Ind, Id) -> F1(Ind, Id) orelse  F2(Ind, Id) end
    end;
