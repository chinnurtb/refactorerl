...
%%%
%%% The module attribute matrix can be created by using the `mod_empty_attrib/0'
%%% and `mod_attrib_data/0' functions:
%%% ```
...
%%% `fun_empty_attrib/0' and `fun_attrib_data/0' functions:
%%% ```
...
%%%
%%% The filter functions `library_mod/2' and `internal_fun/2' can be used with
%%% the `core:filter/3' function:
%%% ```
...
%% @type ref_adder_fun() =
%%           (Fun::fun_attr(),Object::term(),Count::integer(),D::dictionary())
%%           ->  dictionary().
%%
...
add_refs(Funs,PathEnd,Dict,AdderFun) ->
    Path = [{fundef, back},funcl, {scope, back},visib, {sup, back}] ++ PathEnd,
    References =
...
%% @doc Returns the fun_attr record of the `Fun' function.
%%
%% @todo Write a @spec tag.
fun_to_fun_attr(Fun) ->
...
%% @doc Returns the rec_attr record of the `Record' record.
%%
%% @todo Write a @spec tag.
...
%% @doc Returns the macro_attr record of the `Macro' macro.
%%
%% @todo Write a @spec tag.
