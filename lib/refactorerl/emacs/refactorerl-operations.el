...
(define-refac-operation (refactorerl-introduce-import :menu "Introduce Import" :key "ii"
...
2. Call the refactoring from the menu or with \\[refactorerl-introduce-import]."
  (refac-transform 'referl_tr_introduce_import
                   :file buffer-file-name
                   :position pos))

(define-refac-operation (refactorerl-upgrade-regexp-iface :menu "Upgrade regexp interface" :key "ui"
                                                          :precondition :buffer-state)
  ((pos :point))
...
2. Call the refactoring from the menu or with \\[refactorerl-upgrade-regexp-iface]."
  (refac-transform 'referl_tr_upgrade_regexp
                   :file buffer-file-name))

(define-refac-operation (refactorerl-extract-function :menu "Extract function" :key "fe"
                                                      :precondition :buffer-state)
...
(define-refac-operation (refactorerl-inline-function :menu "Inline function" :key "fi"
                                                     :precondition :buffer-state)
  ((pos :point))
...
(define-refac-operation (refactorerl-inline-macro :menu "Inline macro" :key "mi"
                                                     :precondition :buffer-state)
  ((pos :point))
...
2. Call the refactoring from the menu or with \\[refactorerl-inline-macro]."
  (refac-transform 'referl_tr_inline_mac
                   :file buffer-file-name
                   :position pos))

(define-refac-operation (refactorerl-tuple-funpar :menu "Tuple function parameters" :key "ft"
...
(define-refac-operation (refactorerl-reorder-funpar :menu "Reorder function parameters" :key "fo"
                                                    :precondition :buffer-state)
...
(define-refac-operation (refactorerl-generalize-function :menu "Generalize function definition" :key "fg"
                                                         :precondition :buffer-state)
...
(define-refac-operation (refactorerl-expand-funexpr :menu "Expand fun expression" :key "ef"
                                                    :precondition :buffer-state)
  ((pos :point))
...
(define-refac-operation (refactorerl-merge-expr :menu "Merge expressions" :key "me"
...
(define-refac-operation (refactorerl-introduce-rec :menu "Introduce record" :key "ri"
  ...
  (refac-transform 'referl_tr_introduce_rec
                    :file buffer-file-name
                    :posrange (vector beg (1- end))
                    :name name
                    :text fields)
...
3. Type the new header file name."
  (refac-transform 'referl_tr_rename_header
                   :file buffer-file-name
                   :filename name))
...
3. Type the new macro name."
  (refac-transform 'referl_tr_rename_mac
                   :file buffer-file-name
                   :position pos
                   :macname name))
