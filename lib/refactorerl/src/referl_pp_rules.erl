...
erlang_rules() ->
    ...
    %#region % indentations
     [
        ...
        #indentRule{
            ...
            indent = [{from, {sub,2}, % right part
                       to,   last,    % right part
                       1}]},
        #indentRule{
            parent = [{expr,application}],
            indent = [{'after', {elex,1}, % first parameter
                       to,      last,     % cl_paren
                       1}]},
        #indentRule{
            parent = [{expr,record_expr},{expr,record_update}],
            indent = [{from, {elex,2   }, % op_brace
    ...
    %#region % white spaces and required line breakers
     [
        ...
        #tokenRule{
            parents = [{lex, subst}], % Macro subsituation
            rules   = [{[comma,{position,last}], [{ws,0,1}]},
        ...
        #tokenRule{
            ...
            rules   = [{[minus],              [{ws,1,0},{nl,1,0}]},
                       {[stop],               [{ws,0,1},{nl,0,1}]}]},
        #tokenRule{
            parents = [{lex,incl}],
            rules   = [
                {[{check_fun,diff_next_form_gen(-1,lex,include),minus}],
        ...
        #tokenRule{
            ...
            rules   = [{[{before_link,sub,string}],     [{ws,0,1}]} ]},
        #tokenRule{
        ...
        #tokenRule{
            ...
            rules   = [{['cond','let','query'],         [{ws,0,1}] },
                       ...
                       {[hash],                         [{ws,0,0}]},
                       {[vline,dbl_vline,exclam],       [{ws,1,1}]} ]},
        % Clause
        #tokenRule{
            parents = [{clause,fundef},{clause,guard},{clause,timeout},
                       {clause,pattern}],
            rules   = [{[{before_link,body,comma}],     [{ws,0,1},{nl,0,1}]} ]},
        #tokenRule{
            parents = [{clause,funexpr}],
            rules   = [{[{before_link,body,comma}],     [{ws,0,1}]} ]},
        #tokenRule{
            parents = [clause],
            rules   = [{[op_paren,cl_paren],            [{ws,0,0}]},
                       {['when',arrow],                 [{ws,1,1}]},
                       {[comma],                        [{ws,0,1}]} ]},
        % Form
        #tokenRule{
            ...
            rules   = [{[minus],       [{ws,0,0}]},
                       {[stop],        [{ws,0,1},{nl,0,2}]} ]},
        #tokenRule{
            parents = [{form,{attrib,record}},{form,{attrib,vsn}},
                       {form,{attrib,behaviour}},{form,{attrib,file}}],
            rules   = [{[minus],       [{ws,1,0},{nl,2,0}]},
                       {[stop],        [{ws,0,1},{nl,0,2}]} ]},
        #tokenRule{
        ...
        #tokenRule{
            parents = [{form, func}],
            rules   = [{[semicol],     [{ws,0,1},{nl,0,1}]},
                       {[stop],        [{ws,0,1},{nl,0,2}]} ]},
        #tokenRule{
            parents = [{form, {attrib,export}}],
            rules   = [
                {[{check_fun,diff_next_form_gen(-1,attrib,export),minus}],
                 {[{ws,1,0},{nl,2,0}],[{ws,1,0},{nl,1,0}]} },
                {[{check_fun,diff_next_form_gen(+1,attrib,export),stop}],
                 {[{ws,0,1},{nl,0,2}],[{ws,0,1},{nl,0,1}]} } ]},
        #tokenRule{
            parents = [{form, {attrib,import}}],
            rules   = [
                {[{check_fun,diff_next_form_gen(-1,attrib,import),minus}],
                 {[{ws,1,0},{nl,2,0}],[{ws,1,0},{nl,1,0}]} },
                {[{check_fun,diff_next_form_gen(+1,attrib,import),stop}],
                 {[{ws,0,1},{nl,0,2}],[{ws,0,1},{nl,0,1}]} } ]},
        #tokenRule{
            parents = [form],
            rules   = [{[comma],       [{ws,0,1}         ]},
    ...
    %#region % long line breakers
     [
        ...
        #tokenRule{
            parents = [{lex, subst}], % macro subsituation
            rules   = [{[comma],              ?LongNlRules}]},
        #tokenRule{
            parents = [{lex, arg}],   % macro definition parameter
            rules   = [{[comma],              ?LongNlRules}]},
        #tokenRule{
            parents = [{lex, body}],  % macro definition body
            rules   = [{[plus,minus,star,dbl_plus,dbl_minus,
        ...
        % Expression
        #tokenRule{
            parents = [{expr, application}],
            rules   = [{[op_paren, comma],              ?LongNlRules}]},
        #tokenRule{
            parents = [{expr, fun_expr}],
            rules   = [{[semicol],                      ?LongNlRules}]},
        #tokenRule{
            parents = [{expr, mstring}],
            rules   = [{[{before_link,sub,string}],     ?LongNlRules}]},
        #tokenRule{
            parents = [{expr,match_expr}],
            rules   = [{[equal],                        ?LongNlRules}]},
        #tokenRule{
            parents = [expr],
            rules   = [{[comma,vline,dbl_vline,exclam], ?LongNlRules}]},
        % Clause
        #tokenRule{
            parents = [{clause,fundef},{clause, funexpr},{clause,guard},
                       {clause,timeout},{clause,pattern}],
            rules   = [{[arrow],                        ?LongNlRules}]},
        #tokenRule{
            parents = [{clause, funexpr}],
            rules   = [{[{before_link,body,comma}],     ?LongNlRules}]},
        #tokenRule{
            parents = [clause],
            rules   = [{[comma,cl_paren,'when'],        ?LongNlRules}]},
        % Form
        #tokenRule{
            parents = [form],
            rules   = [{[comma],                        ?LongNlRules}]}
    ...
    %#region % terms in multiple lines
     [
        ...
        #tokenRule{
            parents = [{clause, funexpr}],
            rules   = [{[{before_link,body,comma}],    [{nl,0,1}]}]}
     ]
...
dataKeyFields(#clause{kind=K})           -> K;
dataKeyFields(#expr{kind=K})             -> K;
