...
-module(referl_tr_apply_funcluster).
-vsn("$Rev: 0000 $").
-include("refactorerl.hrl").

-export([do/1]).
-export([get_operations/1,commit/1,main/0,
...
do_moves(Tr,Moves) ->
    [begin
        {NameAtom,ListAtom,TrAtom} =
            case Type of
                rec  -> {filename,reclist,referl_tr_move_rec};
                mac  -> {filename,maclist,referl_tr_move_mac};
                func -> {name,    funlist,referl_tr_move_fun}
            end,
...
get_used_recmac(Mod,Parent) ->
    get_used_recmac(Mod, Parent, ?File:records(), {recref,back}) ++
...
% TODO: integrate testcases somehow...
% TEST case #2:
...
%referl_tr_apply_funcluster:do([[{src1,f,1},{src1,k,2},{src2,g,0}],[{src1,h,2}]]).
