...
-export([module/0, func/0, exprs/0, deep_exprs/0, macros/0, records/0]).


%% @spec type(node()) -> atom()
%% @doc Returns the type of a form as an atom. The returned atom is
%% the tag (e.g. export, import) by module attributes and `macro' by
...
%% @doc The result query returns subexpressions directly under the form (mainly 
%% useful in attribute forms)
exprs() -> [attr].
