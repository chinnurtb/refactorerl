...
%%% a textually represented Erlang term. The type of the table is the
%%% following:
%%%
%%% {@type [{State::integer(), {[{Char::char(), NextState::integer()@}],
%%%        Accept::atom()@}@}]}.
%%%
%%% The elements of the list are ordered by `State'.
%%%
...
parse_line(Line, Defs) ->
    case regexp:first_match(Line, "[0-9A-Z_a-z]+") of
        {match, St, Len} ->
            parse_line(string:substr(Line, St, Len),
                       string:substr(Line, St+Len), Defs);
        nomatch ->
            throw("Missing rule name.")
    end.

parse_line(Name, Rest, Defs) ->
    case regexp:first_match(Rest, "[ \t]*=[ \t]*") of
        {match, St, Len} ->
            Re = string:strip(string:strip(
                                string:substr(Rest, St+Len), right, $\n)),
            {Name, parse_regexp(Re, Defs)};
        nomatch ->
            throw("Missing equal sign.")
    end.
