...
-define(TabWidth,
	8).  %% Added by Huiqing Li (THIS SHOULD BE READ FORM EMACS!!).
...
scan([C | Cs], Stack, Toks, {Line, Col}, State, Errors)
    when C == $\t ->
    scan(Cs, Stack, Toks, {Line, Col + (?TabWidth)}, State,
	 Errors);
...
scan([$$ | Cs], Stack, Toks, {Line, Col}, State,
    ...
    scan_char(Cs, Stack, Toks, {Line, Col}, State, Errors);
scan([$' | Cs], _Stack, Toks, {Line, Col}, State,
     Errors) ->      % Quoted atom
    scan_qatom(Cs, [$', {Line, Col}], Toks, {Line, Col},
	       State, Errors);
scan([$" | Cs], _Stack, Toks, {Line, Col}, State,
     Errors) ->      % String
    scan_string(Cs, [$", {Line, Col}], Toks, {Line, Col},
		State, Errors);
scan([$% | Cs], Stack, Toks, {Line, Col}, State,
     Errors) ->       % Comment
    scan_comment(Cs, Stack, Toks, {Line, Col}, State,
		 Errors);
